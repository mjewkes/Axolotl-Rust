...
use self::crypto::hkdf;
use self::crypto::digest::Digest;     // False Warning

const DIGEST_LENGTH : usize = 32;

/// Fills output_key_material with output from HKDF
pub fn derive_key(input_key_material : &[u8], info : &[u8] , salt : &[u8], output_key_material : &mut [u8] ) {
    let digest = Sha256::new();
    let mut pseudo_random_key: [u8; DIGEST_LENGTH] = [0;DIGEST_LENGTH];

    hkdf::hkdf_extract(digest, &salt, &input_key_material[..], &mut pseudo_random_key);
    hkdf::hkdf_expand(digest, &pseudo_random_key[..], &info[..], output_key_material);
}
...
mod tests {
    ...
    use self::crypto::digest::Digest; // False Warning
     ...
     #[test]
     fn rfc5869_kat_1(){
        let ikm: [u8;22]  =    [0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 
                                0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 
                                0x0b, 0x0b];

        let salt: [u8;13] =    [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 
                                0x0a, 0x0b, 0x0c];

        let info: [u8;10] =    [0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9];

        let okm: [u8;42]  =    [0x3c, 0xb2, 0x5f, 0x25, 0xfa, 0xac, 0xd5, 0x7a, 0x90, 0x43, 
        ...
        derive_key(&ikm, &info,&salt,&mut output[..]);
        assert_eq!(&output[..],&okm[..]);
     }

     #[test]
     fn rfc5869_kat_2(){
        let ikm: [u8;80] =     [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 
        ...
        let salt: [u8;80] =    [0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 
        ...
        let info: [u8;80] =    [0xb0, 0xb1, 0xb2, 0xb3, 0xb4, 0xb5, 0xb6, 0xb7, 0xb8, 0xb9, 
        ...
        let okm: [u8;82] =     [0xb1, 0x1e, 0x39, 0x8d, 0xc8, 0x03, 0x27, 0xa1, 0xc8, 0xe7, 
    ...
    fn rfc5869_kat_3(){ 
        ...
        let okm: [u8;L]   =    [0x8d, 0xa4, 0xe7, 0x75, 0xa5, 0x63, 0xc1, 0x8f, 0x71, 0x5f, 
