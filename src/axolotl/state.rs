...
impl <T:Axolotl> ReceiveChain<T> {
    ...
    fn try_decrypt_with_message_key_index(
        ...
        if expected_mac == mac {
            axolotl_impl.with_decoded_ciphertext(message, |ciphertext| {
                axolotl_impl.decrypt_message(&message_key, &ciphertext)
            })
        }
        else {
            None
...
impl <T:Axolotl> AxolotlState<T> {
    ...
    fn encrypt_and_get_next_chain_key(&self, axolotl_impl : &T, plaintext : &T::PlainText) -> (T::ChainKey, (T::Message, T::Mac)) {
        ...
        let message = axolotl_impl.encode_header_and_ciphertext(
            (self.message_number_send, self.ratchet_key_send.public.clone()),
            ciphertext
        );
    ...
    pub fn decrypt(&mut self, axolotl_impl : &T, message : &T::Message, mac : T::Mac) -> Option<T::PlainText> {
        axolotl_impl.with_decoded_header(message, |message_number, message_ratchet_key| {
            ...
            match receive_chain_position {
                ...
                None => {
                    self.try_decrypt_with_new_chain(axolotl_impl, message_number, message_ratchet_key, message, mac)
                }
            }
        })
    }
