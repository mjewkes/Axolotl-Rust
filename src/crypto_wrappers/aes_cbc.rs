...
pub use utils::exceptions::{CryptoError,CryptoErrorKind};
...
pub fn encrypt_aes256_cbc_mode(data: &[u8], key:  [u8; KEY_LEN], iv: [u8;IV_LEN]) -> Result<Vec<u8>,  CryptoError>{
    ...
    loop {
        let result : BufferResult = match encryptor.encrypt(&mut read_buffer, &mut write_buffer, true){
            Err(why) => { return Err(translate_error(why))},
            Ok(val)  => val
        };
...
pub fn decrypt_aes256_cbc_mode(ciphertext: &[u8], key:  [u8; KEY_LEN], iv: [u8;IV_LEN]) -> Result<Vec<u8>,  CryptoError>{
...
fn translate_error(error : SymmetricCipherError) -> CryptoError{
    match error {
        SymmetricCipherError::InvalidLength  => CryptoError::new(CryptoErrorKind::Unknown,"Invalid length"),
        SymmetricCipherError::InvalidPadding => CryptoError::new(CryptoErrorKind::Unknown,"Invalid padding"),
    }
}
...
mod tests {
    ...
    fn cbc_encryption() {
        ...
        let ciphertext = encrypt_aes256_cbc_mode(&data,key,iv).unwrap();
    ...
    fn cbc_roundtrip() {
        ...
        let cipher_text = encrypt_aes256_cbc_mode(&plaintext,key,iv).unwrap();

        let derived_plaintext = decrypt_aes256_cbc_mode(&cipher_text[..],key,iv).unwrap();
