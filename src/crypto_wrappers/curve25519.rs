...
pub fn derive_public_key( private_key: &PrivateKey) -> PublicKey {
    ...
    PublicKey(invoke_curve25519_donna(&**private_key,basepoint))
}

pub fn derive_shared_key( private_key: &PrivateKey, remote_public_key : &PublicKey ) -> SharedKey {
    SharedKey(invoke_curve25519_donna(&**private_key,&**remote_public_key))
}
...
mod tests {
    ...
    fn shared_kat(){
        ...
        let alice_pub  = PublicKey(alice_public_bytes);
        let alice_priv = PrivateKey(alice_private_bytes);

        let bob_pub  = PublicKey(bob_public_bytes);
        let bob_priv = PrivateKey(bob_private_bytes);

        let expected_shared = SharedKey(shared_bytes);
    ...
    #[test]
    fn loop_test(){
        let mut e1 = [0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00];
        let mut e2 = [0x05,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00];
        let mut k  = [0x09,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                      0x00,0x00];
        ...
        for __ in 0 .. LOOP_ITERATIONS {
            ...
            let e1k   = derive_shared_key(&PrivateKey(e1),&PublicKey(k));
            let e2e1k = derive_shared_key(&PrivateKey(e2),&to_pub(&e1k));
            let e2k   = derive_shared_key(&PrivateKey(e2),&PublicKey(k));
            let e1e2k = derive_shared_key(&PrivateKey(e1),&to_pub(&e2k));

            assert_eq!(*e1e2k,*e2e1k);
            for x in 0..32 {
                e1[x] = e1[x] ^ (*e2k)[x];
                e2[x] = e2[x] ^ (*e1k)[x];
                k[x]  = k[x]  ^ (*e1e2k)[x];
